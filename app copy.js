...
function loadData() {
  ...
  const DATA_VERSION = 8; // Bump this when new default content/structure is added
  ...
  if (saved && savedVersion >= DATA_VERSION) {
    ...
    if (!appData.classes) appData.classes = DEFAULT_DATA.classes;
    if (!appData.subjects) appData.subjects = DEFAULT_DATA.subjects;
    if (!appData.topics) appData.topics = DEFAULT_DATA.topics;
    if (!appData.content) appData.content = DEFAULT_DATA.content;
  } else {
    // Fresh load or new version: merge defaults with any user-added content
    appData = JSON.parse(JSON.stringify(DEFAULT_DATA));
    if (saved) {
      const old = JSON.parse(saved);
      // Keep user-added content (IDs starting with 'c-' or 'topic-')
      if (old.content) {
        const userContent = old.content.filter(c => c.id.startsWith('c-'));
        appData.content = appData.content.concat(userContent);
      }
      if (old.topics) {
        const userTopics = old.topics.filter(t => t.id.startsWith('topic-'));
        appData.topics = appData.topics.concat(userTopics);
      }
    }
